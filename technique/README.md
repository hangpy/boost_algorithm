...
[vector(배열)에서 중복 원소 제거하기](#vector(배열)에서-중복-원소-제거하기)
...
`<algorithm>`에서 오름차순으로 `sort()` 후 `unique()` 함수를 쓰면 수월하게 중복원소를 제거할 수 있다. 

```cpp
int arr[10] = { 4,1,5,5,3,2,3,4,5,1 };

// 반드시 sort부터 하고나서
// 유니크한 원소의 개수를 뽑기 위한 기법
// unique() -> 원소 모두 뽑고 그 다음 위치를 반환
sort(arr, arr + 10);

// n은 중복을 제거했을 때 원소의 개수를 의미한다.
// unique()는 중복을 제외하고 앞으로 모두 땡긴 후 그 다음 위치를 반환 (vector::end()를 생각)
// 또한 unique()는 배열을 직접 건드리기 때문에 주의해야함.
int n = unique(arr, arr + 10) - arr;

for (int i = 0; i < 10; i++) cout << arr[i] << " \n"[i == 9];
// 결과: 1 2 3 4 5 4 4 5 5 5
// 즉, unique()는 6번째 위치 즉 5, arr[5](벡터의 경우, 반복자)의 위치를 반환한다.
```

[참고자료](https://en.cppreference.com/w/cpp/algorithm/unique)
